...
setup_host.sh 初始化流程：
  1. xdp_prog_user load xdp_prog_kern.o
       → bpf_object__load()：一次性加载所有程序，创建共享 maps
       → 逐个 bpf_map__pin()：maps 扁平 pin 到 /sys/fs/bpf/xdp_ipip_*
       → bpf_program__pin()：pin pod_egress_prog 和 eth_ingress_prog
  2. ip link set dev ens33 xdp pinned /sys/fs/bpf/xdp_ipip_eth_ingress_prog
       → 复用已 pin 的程序实例（共享同一组 maps）

add_pod.sh 添加 pod 流程：
  1. 创建 netns + veth
  2. ip link set dev <pod-host> xdp pinned /sys/fs/bpf/xdp_ipip_pod_egress_prog
       → 复用同一程序实例，与 eth_ingress 共享全部 maps
  3. xdp_prog_user deliver/txport/route → 更新共享 maps
```

所有程序实例操作同一组 pinned maps，更新立即对所有接口生效。无需 bpftool，仅依赖 libbpf 和 iproute2 `ip`。
...
| clang / llvm | 10+ |
| libbpf-dev | 0.5+ |
| iproute2 | 任意现代版本（`ip link set xdp`） |

> **无需 bpftool**：程序加载、map 操作均由 `xdp_prog_user` 通过 libbpf API 直接完成。
...
sudo bash scripts/add_pod.sh pod6 10.244.3.11
...
# VM1 上：
sudo bash scripts/add_remote_route.sh 10.244.3.10 192.168.1.3 $VM3_MAC
sudo bash scripts/add_remote_route.sh 10.244.3.11 192.168.1.3 $VM3_MAC
...
```bash
# 加载 BPF 程序，pin maps 和 progs 到 /sys/fs/bpf/
sudo ./xdp_prog_user load xdp_prog_kern.o

# 读取本机宿主信息（输出 "ip mac"）
sudo ./xdp_prog_user host get

# 设置宿主机信息
sudo ./xdp_prog_user host    set <host_ip> <eth_ifname|ifindex> <eth_mac>

# 添加路由条目
sudo ./xdp_prog_user route   add <pod_ip> <host_ip> <host_mac>

# 添加本地投递条目
sudo ./xdp_prog_user deliver add <pod_ip> <ifname|ifindex> <pod_mac>

# 注册 DEVMAP 端口
sudo ./xdp_prog_user txport  add <ifname|ifindex>
...
Pinned 路径（扁平挂载于 `/sys/fs/bpf/`）：

```
/sys/fs/bpf/xdp_ipip_routing_map
/sys/fs/bpf/xdp_ipip_delivery_map
/sys/fs/bpf/xdp_ipip_host_config
/sys/fs/bpf/xdp_ipip_tx_ports
/sys/fs/bpf/xdp_ipip_pod_egress_prog
/sys/fs/bpf/xdp_ipip_eth_ingress_prog
```
...
| Map 共享 | `bpf_object__load` 一次性加载 + pin + `ip link set xdp pinned` | 同一宿主机上所有 XDP 程序共享转发表，无需 bpftool |
...
├── xdp_prog_user.c            # 用户态工具（load/route/deliver/host/txport/dump，libbpf）
│
├── common/
...
```
初始部署：
  scripts/setup_host.sh  ──→  挂载 bpffs、加载程序（xdp_prog_user load）、挂载 xdp_eth_ingress
       │
       ▼（每个 pod 执行一次）
  scripts/add_pod.sh     ──→  创建 netns + veth、挂载 xdp_pod_egress（ip xdp pinned）、更新 maps
       │
       ▼（在每台远程宿主机上执行）
  scripts/add_remote_route.sh ──→  更新远程宿主机的 routing_map

动态扩容：
  scripts/add_pod.sh + scripts/add_remote_route.sh（在所有其他宿主机上）

缩容：
  scripts/del_pod.sh + ./xdp_prog_user route del（在所有其他宿主机上）

新增宿主机：
  scripts/setup_host.sh + scripts/add_pod.sh + 双向 scripts/add_remote_route.sh
